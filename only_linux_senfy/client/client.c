...
int recv_file_from_server(int sockfd, const char* save_file_loc);
void print_help();
...
int main(int argc, char *argv[]) {
    ...
    bzero(&server_addr, sizeof(server_addr)); // zeroing all bytes
 
    // assigning IP and PORT numbers
    server_addr.sin_family = AF_INET;
    in_addr_t ip_address = inet_addr(confirmed_arguments.ip_address_string);
    if (ip_address == INADDR_NONE) {
        fprintf(stderr, "Failed: converting IPv4 address string to valid IPv4\n");
        close(sockfd);
        return -1;
    }
    server_addr.sin_addr.s_addr = ip_address; // server address to connect to
    ...
    if (connect(sockfd, p_serv_addr, sizeof(server_addr)) != 0) {
...
int new_file_location_valid(char *new_file_save_loc, size_t size_new_file_save_loc) {
    ...
    struct stat statbuf;
    if (!stat(dir_path, &statbuf) == 0 && S_ISDIR(statbuf.st_mode)) { // parent directory does not exist
...
int recv_file_from_server(int sockfd, const char* save_file_loc) {
    ...
    ssize_t num_bytes_recv; // signed int
    int flags = 0;
