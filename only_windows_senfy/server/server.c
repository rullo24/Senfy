...
void print_help();
int send_file_to_client(int connfd, const char* filename);
...
int main(int argc, char* argv[]) {
    ...
    socklen_t size_client_info = sizeof(client_info); // storing byte size of addr info struct
    ...
    int connfd = accept(sockfd, p_client_info, &size_client_info); 
    if (connfd < 0) {
...
int check_arguments(int argc, char* argv[], server_nec_args *p_confirmed_arguments) {
    ...
    if (access(p_confirmed_arguments->filename_string, F_OK) == -1) {
